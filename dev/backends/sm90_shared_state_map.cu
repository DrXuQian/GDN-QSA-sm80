...
using Types=gdn::sm90::ValueKernelTypes<BF16,false,128,232,true>;
...
void map(int plant=0) {
    ...
    std::array<std::array<int,128>,128> values{},destinations{};
    for(int tid=0;tid<128;++tid) {
        ...
        need(size(src)==128 && size(dst)==128);
        for(int i=0;i<128;++i) { values[tid][i]=src(i);destinations[tid][i]=dst(i); }
    }
...
template<class Gate,bool Initial> void actual_type() {
    using T=gdn::sm90::ValueKernelTypes<Gate,Initial,128,232,true>;
