...
families=0
for flag in "${DELIVERY_AB:-0}" "${TILE_AB:-0}" "${STATE_AB:-0}" "${STAGE_AB:-0}" "${PREPARE_ROWS_AB:-0}"; do
...
if (( families > 1 )); then
  echo "[WY box] FAIL: choose only one of DELIVERY_AB / TILE_AB / STATE_AB / STAGE_AB / PREPARE_ROWS_AB" >&2
  exit 1
fi
