...
#include "scalar_gate.cuh"
#include "abi_layout.cuh"
...
struct FlatMainloopTmaWarpSpecializedKdaFwd {
    ...
    using ElementGateGmem = find_option_t<Tag::kElementGateGmem, float, Options>;
    ...
    // in global nor shared memory. Alpha is only an elementwise operand, never
    // a WGMMA descriptor; CuTe derives scalar/broadcast S2R copies from stride0.
    using SmemLayoutAlpha_SD = decltype(make_layout(
        make_shape(BlkSeqKV, make_shape(_32{},Int<HeadSizeQK/32>{}), Int<StagesAlpha::value>{}),
        make_stride(_1{}, make_stride(_0{},_0{}), BlkSeqKV)));
    // raw layout for copy
    ...
    using MainloopAlphaPipeline = std::conditional_t<NeedsAlpha, cutlass::PipelineAsync<StagesAlpha::value>, Unused>;
    ...
    ElementGateGmem const* ptr_Gate; // scalar natural-log decay [B,T,Hv]
    ...
    struct Params {
        ...
        TMA_K tma_load_k;
        TMA_V tma_load_v;
        ElementGateGmem const* gate_ptr;
        TMA_O tma_store_o;
        void* output_base;
        float scale;
    ...
    to_underlying_arguments(ProblemShape const& problem_size, Arguments const& args, void* workspace) {
        ...
        return Params{
            ...
            .gate_ptr = args.ptr_Gate,
            .tma_store_o = params_o.tma_store_o,
            .output_base = params_o.output_base,
            .scale = args.scale,
    ...
    load_qkv(
        ...
        for (int blk = 0; blk < num_blocks; ++blk) {
            // Chunk-local natural-log scalar GDN gate -> cuLA's log2 prefix image.
            gdn::sm90::load_scalar_gate<BlkSeqQ, HeadSize>(
    ...
    store(
        ...
        auto collective_store = CollectiveStoreO{tma_store, pipeline, storage, output_base};
    ...
    compute(
        ...
        auto kv_load = [&](auto& tKVrKV) INLINE_LAMBDA {
            ...
            auto gKV = make_tensor(
                ...
                gdn::sm90::state_layout<HeadSizeQK,HeadSizeV>(num_state_heads,problem_size.num_seqs))(
                _, _, state_head_idx, seq_idx);  // (KDim, VDim), V-contiguous
        ...
        auto kv_store = [&]() INLINE_LAMBDA {  // tKVrKV is carried over whole mainloop
            ...
            auto gKV = make_tensor(
                ...
                gdn::sm90::state_layout<HeadSizeQK,HeadSizeV>(num_state_heads,out_num_seqs))(
                _, _, state_head_idx, out_seq_idx);  // (KDim, VDim), V-contiguous
