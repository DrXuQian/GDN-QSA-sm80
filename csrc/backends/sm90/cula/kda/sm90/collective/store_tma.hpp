...
struct CollectiveStoreTma {
  ...
  using SmemLayoutAtom = decltype(cutlass::epilogue::collective::detail::sm90_get_epilogue_smem_swizzle_layout_atom<
    ...
    CUTE_DEVICE
    CollectiveStoreTma(TMA const& tma_store, Pipeline& pipeline, SharedStorage& storage, void* output_base)
        : tma_store_(tma_store), pipeline_(pipeline), storage_(storage), output_base_(output_base) {
    }
    ...
    get_workspace_size(/*Arguments const& args,*/ int sm_count) {
        ...
        size_t num_bytes = 0; // fixed batches: no device tensor-map mutation
    ...
    step(
        ...
        if (can_process(problem_size, work_desc, dst_iter, num_iters)) {
            ...
            // A fixed batch tail must not overwrite the next sequence.
            auto shared = make_tensor(make_smem_ptr(storage_.data()), SmemLayoutO{});
            int lane = cutlass::canonical_lane_idx();
            int rows = int(work_desc.seq_len) - dst_iter * int(SizeN{});
            CUTE_NO_UNROLL
        ...
        // Retire shared readers before the producer may reuse the O stage.
