...
std::vector<torch::Tensor> forward(torch::Tensor q, torch::Tensor k, torch::Tensor v,
  ...
  static_assert(Variant <= 6 && (!Variant || Residual), "invalid residual-only delivery");
  ...
  if constexpr (Residual) {
    if constexpr (Variant != 0) {
      constexpr auto launch = Variant == 1 ? gdn_wy_forward_residual_prefetch :
                              ...
                              Variant == 5 ? gdn_wy_forward_residual_warps8 : gdn_wy_forward_residual_warps8_blayout;
