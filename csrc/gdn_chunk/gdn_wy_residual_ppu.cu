...
gdn_wy_residual_state(Inputs p, Workspace ws, float* final) {
...
extern "C" int gdn_wy_forward_residual(
  ...
  if (rc) return rc;
  rc = int(hggcFuncSetAttribute(gdn_wy_residual_state,
  ...
  gdn_wy_residual_state<<<grid, Plan::Threads, sizeof(Storage), stream>>>(p, ws, final);
