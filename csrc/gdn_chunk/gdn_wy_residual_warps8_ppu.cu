...
gdn_wy_residual_warps8_state(Inputs p, Workspace ws, float* final) {
...
extern "C" int gdn_wy_forward_residual_warps8(
  ...
  using namespace residual_warps8;
  ...
  if (rc) return rc;
  rc = int(hggcFuncSetAttribute(gdn_wy_residual_warps8_state,
  ...
  gdn_wy_residual_warps8_state<<<grid, Plan::Threads, sizeof(Storage), stream>>>(p, ws, final);
