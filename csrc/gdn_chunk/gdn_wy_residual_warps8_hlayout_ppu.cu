...
gdn_wy_residual_warps8_hlayout_state(Inputs p, Workspace ws, float* final) {
...
gdn_wy_residual_warps8_hlayout_output(Inputs p, Workspace ws, BF16* output) {
  using Q = aiu::Tile<Chunk, Dim>;
  using A = aiu::Tile<Chunk, Chunk>;
  using H = OutputSnapshot;
  using V = aiu::Tile<Chunk, OutputTile::Panel>;
  extern __shared__ __align__(128) unsigned char storage[];
...
int configure_hlayout_output() {
  return int(hggcFuncSetAttribute(gdn_wy_residual_warps8_hlayout_output,
      hggcFuncAttributeMaxDynamicSharedMemorySize, sizeof(TiledOutputStorage)));
}
int launch_hlayout_output(Inputs p, Workspace ws, BF16* output, gdn_arch::Stream stream) {
  gdn_wy_residual_warps8_hlayout_output<<<unsigned(p.shape.groups()), OutputTile::Threads,
...
extern "C" int gdn_wy_forward_residual_warps8_hlayout(
  ...
  using namespace residual_warps8_hlayout;
  ...
  if (rc) return rc;
  rc = int(hggcFuncSetAttribute(gdn_wy_residual_warps8_hlayout_state,
      hggcFuncAttributeMaxDynamicSharedMemorySize, sizeof(Storage)));
  if (rc) return rc;
  rc = configure_hlayout_output();
  if (rc) return rc;
  ...
  gdn_wy_residual_warps8_hlayout_state<<<grid, Plan::Threads, sizeof(Storage), stream>>>(p, ws, final);
  rc = int(hggcGetLastError());
  if (rc) return rc;
  return launch_hlayout_output(p, ws, static_cast<BF16*>(output), stream);
}
