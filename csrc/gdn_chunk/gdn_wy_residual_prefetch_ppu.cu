// Opt-in scheduling-only residual control: prefetch next P behind current H update.
// Arithmetic/rounding/layout remain identical to gdn_wy_residual_ppu.cu.
// Keep the original body compiled independently as the RAW-BIT counterfactual.
...
gdn_wy_residual_prefetch_state(Inputs p, Workspace ws, float* final) {
  ...
  // P0 is independent of H and is committed before the first chunk.
  Inverse::stage(sm.inverse, ws.w + Plan::inverse_base(p.shape.group(b, h, 0)), Chunk, Chunk);
  cute::cp_async_fence();
  #pragma unroll 1
  for (int ct = 0; ct < p.shape.chunks(); ++ct) {
    ...
    for (int k = 0; k < Chunk; k += 16) {
      uint32_t residual[4];
      Value::load<true>(sm.residual, k, StateTile::column(warp), residual);
      CUTE_UNROLL
      for (int r = 0; r < StateTile::ValueFragments; ++r) {
        uint32_t inverse[4];
    ...
    __syncthreads();  // VALUES_READY also retires every P@R reader.
    int const next = Plan::next(ct, p.shape.chunks());
    if (next >= 0) {
      // Single buffer: P is dead now; K/scaledV remain live for UPDATE.
      Inverse::stage(sm.inverse, ws.w + Plan::inverse_base(p.shape.group(b, h, next)), Chunk, Chunk);
      cute::cp_async_fence();
    }
    ...
    for (int r = 0; r < Chunk; r += 16) {
      uint32_t scaled[4];
      Value::load<true>(sm.scaled, r, StateTile::column(warp), scaled);
      CUTE_UNROLL
      for (int k = 0; k < StateTile::KFragments; ++k) {
        uint32_t key[4];
...
extern "C" int gdn_wy_forward_residual_prefetch(
  ...
  using namespace residual_prefetch;
  ...
  if (rc) return rc;
  rc = int(hggcFuncSetAttribute(gdn_wy_residual_prefetch_state,
  ...
  gdn_wy_residual_prefetch_state<<<grid, Plan::Threads, sizeof(Storage), stream>>>(p, ws, final);
