// Private-register metadata lookahead on the measured HV control.
// Matrix layouts/math and the existing HV output are reused unchanged.
...
namespace gdn_qsa::wy::residual_warps8_metadata {
using residual_warps8_hvlayout::StateTile;
using residual_warps8_hvlayout::Plan;
using residual_warps8_hvlayout::Storage;
using residual_warps8_hvlayout::Key;
using residual_warps8_hvlayout::Inverse;
using residual_warps8_hvlayout::Value;
using residual_warps8_hvlayout::Snapshot;
using residual_warps8_hvlayout::PublishedValue;
using residual_warps8_hvlayout::BIntermediate;

struct PrefetchedMetadata { float prefix; BF16 beta; };
__device__ __forceinline__ PrefetchedMetadata prefetch_metadata(
    Inputs const& p, Workspace const& ws, int b, int h, int ct, unsigned tid) {
  auto const at = MetadataPlan::slot(p.shape, b, h, ct, tid);
  PrefetchedMetadata item{0.0f, BF16::bitcast(0)};
  if (at.read_prefix) item.prefix = ws.gates[at.prefix];
  if (at.read_beta) item.beta = p.beta[at.beta];  // keep BF16 bits until publication
  return item;
}

__global__ void __launch_bounds__(Plan::Threads)
gdn_wy_residual_warps8_metadata_state(Inputs p, Workspace ws, float* final) {
  ...
  auto metadata = prefetch_metadata(p, ws, b, h, 0, tid);  // PROLOGUE
  #pragma unroll 1
  for (int ct = 0; ct < p.shape.chunks(); ++ct) {
    ...
    if (tid < Chunk) {
      sm.gates[tid] = metadata.prefix;
      sm.beta[tid] = float(metadata.beta);  // PUBLICATION: prior RETIRE protects old readers
    }
    CUTE_UNROLL
    for (int k = 0; k < StateTile::KFragments; ++k) {
      CUTE_UNROLL
    ...
    if (MetadataPlan::has_next(p.shape, ct))
      metadata = prefetch_metadata(p, ws, b, h, ct + 1, tid);  // LOOKAHEAD before UPDATE
...
extern "C" int gdn_wy_forward_residual_warps8_metadata(
  ...
  using namespace residual_warps8_metadata;
  ...
  if (rc) return rc;
  rc = int(hggcFuncSetAttribute(gdn_wy_residual_warps8_metadata_state,
  ...
  gdn_wy_residual_warps8_metadata_state<<<grid, Plan::Threads, sizeof(Storage), stream>>>(p, ws, final);
