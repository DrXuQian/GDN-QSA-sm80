...
gdn_wy_residual_warps8_operands_state(Inputs p, Workspace ws, float* final) {
...
extern "C" int gdn_wy_forward_residual_warps8_operands(
  ...
  using namespace residual_warps8_operands;
  ...
  if (rc) return rc;
  rc = int(hggcFuncSetAttribute(gdn_wy_residual_warps8_operands_state,
  ...
  gdn_wy_residual_warps8_operands_state<<<grid, Plan::Threads, sizeof(Storage), stream>>>(p, ws, final);
