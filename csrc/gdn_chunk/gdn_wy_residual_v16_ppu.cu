...
gdn_wy_residual_v16_state(Inputs p, Workspace ws, float* final) {
...
extern "C" int gdn_wy_forward_residual_v16(
  ...
  if (rc) return rc;
  rc = int(hggcFuncSetAttribute(gdn_wy_residual_v16_state,
  ...
  if (rc) return rc;
  unsigned const grid = unsigned(int64_t(batch) * value_heads * (Dim / residual_v16::ValueTile));
  gdn_wy_residual_v16_state<<<grid, Plan::Threads, sizeof(Storage), stream>>>(p, ws, final);
